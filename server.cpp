...
int main (int argc, char* argv[]) {
	...
	while (true) {
		sockfd = control_channel->accept_conn();
		TCPRequestChannel* control_channel2 = new TCPRequestChannel(sockfd);
		
		thread thread_for_client(handle_process_loop, control_channel2);
